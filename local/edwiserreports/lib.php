...
function local_edwiserreports_output_fragment_userslist($args) {
    ...
    switch ($page) {
        case "activeusers":
            ...
            $response = \local_edwiserreports\blocks\activeusersblock::get_userslist_table($filter, $action, $cohortid);
            break;
        case "courseengage":
            $courseid = clean_param($args['courseid'], PARAM_TEXT);
            $action   = clean_param($args['action'], PARAM_TEXT);

            $response = (new \local_edwiserreports\blocks\courseengagementblock())->get_userslist_table(
                $courseid,
                $action,
                $cohortid
            );
...
/**
 * Get for report capabilty from
 * @param array $report Fragment parameter data object
 */
function local_edwiserreports_output_fragment_get_reportscap_form($report) {

    $reportname = isset($report['name']) ? $report['name'] : false;
    $component = 'local_edwiserreports';

    if (!$reportname) {
        throw new moodle_exception('invalidreport', 'local_edwiserreports');
    }

    // Check if block is exist or not.
    if (!class_exists("\\local_edwiserreports\\reports\\" . $reportname)) {
        throw new moodle_exception('reportnotexist', 'error');
    }

    // Get report capabilities.
    $capabilities = \local_edwiserreports\utility::get_report_capability($reportname);
    $single = count($capabilities) < 2;
    $capvalue = 'report/edwiserreports_' . $reportname . ':view';

    // Prepare form for report editing.
    ...
    $output .= local_edwiserreports_block_overview_display($capvalue, $reportname);
...
function local_edwiserreports_output_fragment_email_schedule_tabs($args) {
    ...
    $data['graphical'] = $data['graphical'] ? 1 : 0;
    $data['defaultformat'] = $data['graphical'] ? 'pdfimage' : 'csv';
    $data['defaultformatlabel'] = get_string('format' . $data['defaultformat'], 'local_edwiserreports');
    $options = $blockbase->get_block_download_options($data['graphical'], false);
    foreach ($options as $key => $option) {
        $option['label'] = get_string('format' . $option['name'], 'local_edwiserreports');
        $options[$key] = $option;
    }
    $data['format'] = $options;
...
function local_edwiserreports_block_overview_display($capvalue, $blockname = '') {
    ...
    $output = html_writer::start_tag('table', array('class' => 'comparisontable w-full w-100'));
    ...
    foreach ($roles as $role) {
        ...
        // Commented as this code is reversing not allowed roles values and 
        // showing them as green in custom reports change capability
...
function local_edwiserreports_extend_navigation(navigation_node $nav) {
    ...
    // Removing edwiserform link from navigation.
    if ($CFG->branch >= 400) {
        if (stripos($CFG->custommenuitems, "/local/edwiserreports/") !== false) {
            $nodes = explode("\n", $CFG->custommenuitems);
            $itemkey = null;
            foreach ($nodes as $key => $node) {
                if (stripos($node, "/local/edwiserreports/") !== false) {
                    $itemkey = $key;
                }
            }
            unset($nodes[$itemkey]);
